...
> 抽屉布局已由 `docs/superpowers/specs/2026-09-14-right-drawer-redesign-design.md` §8 取代：取消「运行中 / 排队中 / 等待中的文件」分组标题，解析行拆成主行/次行。按钮名与底栏格式仍按本节和下方 Copy rules。
...
允许：运行中心、智能体、文档解析、未启动、正在加载、正在解析、空闲、常驻、释放、停止解析、取消加载、重新加载、识别质量、电子文本、基础解析、印章、公式、图表、导入离线包。不要把「等待中的文件」再写成运行中心分组标题（2026-09-14 §8）。
