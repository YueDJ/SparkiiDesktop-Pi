...
describe('proposal', () => {
  it('freezes payload hash at creation', () => {
    const p = createProposal({ requestId: 'r1', toolName: 'report.export', targetSystem: 'report', summary: 'x', payload: { title: 'r' }, risk: 'write' }, { profileId: 'p1', sessionId: 's1' });
  ...
  it('rejects illegal transition denied→executed', () => {
    const p = transition(createProposal({ requestId: 'r1', toolName: 't', targetSystem: 's', summary: '', payload: {}, risk: 'write' }, { profileId: 'p', sessionId: 's' }), 'denied');
  ...
  it('copies preview from the request and keeps it out of the payload hash', () => {
    ...
    const withPreview = createProposal(
      { requestId: 'r1', toolName: 'write', targetSystem: 'general', summary: '写入 a.txt', preview, payload, risk: 'write' },
      { profileId: 'p', sessionId: 's' },
    );
    const otherPreview = createProposal(
      { requestId: 'r1', toolName: 'write', targetSystem: 'general', summary: '写入 a.txt', preview: { kind: 'text', lines: ['other'] }, payload, risk: 'write' },
      { profileId: 'p', sessionId: 's' },
    );
