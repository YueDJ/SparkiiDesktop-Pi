import { AgentGlyph } from '../icons/index.js';
...
export function AgentNav({ agents, active, onNavigate }: { agents: Array<{ id: string; name: string; status: AgentNavStatus; queuePosition?: number }>; active: string; onNavigate(id: string): void }) {
  return (
    <nav className="ui-agent-nav" aria-label="智能体">
      {agents.map((a) => (
        <button key={a.id} type="button" className={`ui-agent ${active === a.id ? 'on' : ''}`} data-testid={`agent-nav-${a.id}`} onClick={() => onNavigate(a.id)}>
          <AgentGlyph id={a.id} className="ui-agent-glyph" />
          <span>{a.name}</span>
