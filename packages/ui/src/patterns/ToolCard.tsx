...
export function ToolCard({ toolName, input, result, awaitingApproval = false, detail, summary }: { toolName: string; input: unknown; result?: unknown; awaitingApproval?: boolean; detail?: ReactNode; summary?: string }) {
  ...
  return (
    <div className={`ui-tool-card ui-tool-card--${status}`} data-testid="tool-card">
      <div className="ui-tool-card-head"><b>{toolName}</b>{summary && <span className="ui-tool-card-summary">{summary}</span>}<span className="ui-tool-card-status">{statusLabel}</span></div>
